...
interface RecommendationDao {
    ...
    @Query("SELECT * FROM recommendations ORDER BY createdAt DESC, id DESC LIMIT 1")
    fun observeLatestRecommendation(): Flow<RecommendationEntity?>

    @Query("SELECT * FROM recommendations ORDER BY createdAt DESC, id DESC LIMIT 1")
    suspend fun getLatestRecommendation(): RecommendationEntity?
}
